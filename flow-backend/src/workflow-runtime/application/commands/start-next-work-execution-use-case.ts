...
export class StartNextWorkExecutionUseCase {
  ...
  async execute(command: StartNextWorkExecutionCommand): Promise<StartNextWorkExecutionResult> {
    // TRY: Load, validate, and prepare WorkExecution
    ...
    try {
      ...
      compensations.push(async () => {
        try { await this.fileSystem.deleteDirectory(workSpacePath); } catch { /* best-effort */ }
      });
      ...
      compensations.push(async () => {
        try { await this.agentService.stopSession(workExecution.id); } catch { /* best-effort */ }
        try { await this.agentService.deleteSession(workExecution.id); } catch { /* best-effort */ }
      });
