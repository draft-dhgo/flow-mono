...
} from '@workflow-runtime/application/commands/pause-workflow-run-use-case.js';
import { WorkflowRun } from '@workflow-runtime/domain/index.js';
import type { WorkflowRunRepository, EventPublisher } from '@workflow-runtime/domain/index.js';
...
function createUseCase(mocks: ReturnType<typeof createMocks>) {
  return new PauseWorkflowRunUseCase(
...
describe('PauseWorkflowRunUseCase', () => {
  it('pauses a running workflow run', async () => {
  ...
  it('throws WorkflowRunCannotPauseError for non-running run', async () => {
    const mocks = createMocks();
    const run = WorkflowRun.create({
      workflowId: WF_ID,
      issueKey: 'TEST-001',
    ...
    // INITIALIZED state - cannot pause
    vi.mocked(mocks.workflowRunRepository.findById).mockResolvedValue(run);

    const useCase = createUseCase(mocks);
    await expect(
      useCase.execute({ workflowRunId: run.id }),
    ).rejects.toThrow(WorkflowRunCannotPauseError);
  });
});
